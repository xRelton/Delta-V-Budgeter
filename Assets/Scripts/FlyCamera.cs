...
public class FlyCamera : MonoBehaviour {
    ...
    void Update() {
        ...
        if ((Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.A)) && gameObject.transform.position.x > -400) {
            PositionChange.x -= 2;
        }
        if ((Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.D)) && gameObject.transform.position.x < 400) {
            PositionChange.x += 2;
        }
